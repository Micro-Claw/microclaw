...
| 62d | `design62/inspect-artifacts-discovery` | `fcd7058` (2026-09-01) | **assigned 2026-09-01.** Baseline re-taken at `54991bf`, whose code the branch point only adds documentation to: 2738 / 99 / 2, exit 0. Findings handed to the runner as F6 — `max_files` must keep bounding files *examined* or acceptance test 13 passes vacuously; ordering is byte order at two sites and Decision 5 wants case-insensitive; and **`fnmatch` answers differently on Windows than on macOS**, so the case-sensitivity of `name_glob` is decided in the block (case-insensitive everywhere) rather than discovered on the demo machine | **demo machine, one limb.** Windows basename matching, case-insensitive order and a real `Downloads` subtree; everything else settles on macOS. Runs as a script against a fake built from `pathlib`/`os.scandir` behaviour, not from our caller — the lesson design/60's gate paid for | |
...
| 62d | **assigned 2026-09-01**, branch `design62/inspect-artifacts-discovery` off `fcd7058`, runner in its own worktree. Decision 5, acceptance tests 10–14. Read F6 before reviewing it. Its demo-machine limb is the only gate design/62 asks anyone to run |
