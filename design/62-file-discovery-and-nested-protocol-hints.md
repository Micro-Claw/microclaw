...
| 62d | `design62/inspect-artifacts-discovery` | `fcd7058` (2026-09-01) | Turn 1 `9a9ddeb`, no kills. Revision 1 `ddb60a8` (amended, not a second commit) closed all four findings. Coordinator commits `fa2f037` (the `scope` type collision) and `ed757cd` + `2718898` (gate, self-test, runbook). **Coordinator-run suite: 2755 passed / 99 skipped / 2 warnings, baseline +17, zero failures.** **Evidence, coordinator-watched:** the new file against turn 1's product gives `3 failed / 13 passed`, and against revision 1's it gives `2 failed / 15 passed` with the collision printed as `assert 'top-level' == {'name_glob': '*.tif', 'recursive': False, ...}`. **Findings:** the two new arguments could each write a SHA-256 manifest covering 2 of 3 files on disk with **no key recording the narrowing** — measured — which is Decision 5's own *"a partial hash set must never look like a complete manifest"* reached by a route the bound refusal does not cover; `max_files` had **no description at all**, leaving the examined-not-matched rule and the truncated-negative reading unpublished in a block that exists because a parameter was accepted and not published; the unreadable-directory limb used `chmod(0)` and the literal `"Permission denied"`, unique in the suite, against the repo's own `skipif(os.name == "nt", reason="chmod writes no ACL on Windows")` convention at three existing sites, on the one block whose gate runs on Windows; and `scope` was a string in discovery and an object in provenance — F4's defect moved from the request into the response | **demo machine, one limb — PUSHED, NOT YET RUN.** Four limbs, as a program: Windows path resolution and traversal, the returned candidate round-tripping as an input, a bound being distinguishable from an absence in its motivating folder, and **junctions not being followed** — which design/62 asserts is "already true" and nobody had tested, about a Windows-only object, in a repo design/58 already bitten by a junction. Two claims the runner nominated are deliberately not limbs: case-insensitive matching and ordering are pure Python over `entry.name` and could not fail on Windows. Self-tested on macOS by forcing the gate's own `IS_WINDOWS`; that found **three** gate defects before the operator — a `\U` escape making it a syntax error on line 1, an `os.name` mutation making `pathlib` build `WindowsPath`, and a control that did not fire because `rglob` does not cross a symlinked directory | |
