...
**No microscope, and none was booked — that held.** One *demo-machine* gate ran,
for block 62d, and it was worth it: it found a bounds defect (F8) that no amount
of macOS testing could have reached. Everything else is settled off-rig: two are pure-Python ordering, one is
...
exit 0. Navigate by symbol and by the code snippets quoted here; F2's staleness
applies to Decision 5's line numbers too.

**The real signature.** `inspect_artifacts(ctrl, guard, paths, manifest_path=None,
max_files=1000, max_total_bytes=1024**3, max_depth=16, hash=True)`. Neither
`name_glob` nor `recursive` exists, as Decision 5 says. Its sketch also omits
`max_total_bytes`, which is real, is enforced only when `hash` is true, and must
survive.

**`max_files` bounds the accumulated file *set*, not files examined.** Today
those are the same number, because every regular file is added to `files`. The
moment `name_glob` filters, the natural implementation — add only matches —
silently redefines `max_files` from "examined" to "matched". That contradicts
Decision 5's explicit statement that the meaning is unchanged, and it makes
**acceptance test 13 unreachable**: a match lying beyond `max_files` could never
exist, so the limb that stops a bound being reported as a fact about the folder
would pass vacuously. Keep an examined counter distinct from the matched set,
and bound on the examined one.

**Ordering is byte order today, at two sites, and Decision 5 requires
case-insensitive.** `sorted(directory.iterdir(), key=lambda path: str(path))` in
the traversal and `sorted(files, key=lambda p: str(p))` in the result. Measured:
byte order gives `['Beta.ilp', 'Zebra.ilp', 'apple.ilp']`, case-insensitive gives
`['apple.ilp', 'Beta.ilp', 'Zebra.ilp']`. Use a key of
`(str(p).lower(), str(p))` so two names differing only in case stay
deterministic — possible on Linux, not on Windows or default macOS, and free to
handle.

**`fnmatch` answers differently on the demo machine than in the test suite.**
`fnmatch.fnmatch` normalises case through `os.path.normcase`, which is the
identity on darwin and lowercases on Windows. Measured here:

```text
sys.platform == 'darwin'
os.path.normcase('M5.ilp')                            -> 'M5.ilp'
fnmatch('260825_mito_m5.ilp', '*M5*.ilp')             -> False
```

On the demo machine that same call returns **True**. So a `name_glob` built on
bare `fnmatch.fnmatch` would make the macOS suite and the one gate this block
ships disagree about the same input — found off-rig, which is where design/59
says this class of thing must be settled.

**Decide it in the block:** match **case-insensitively on every platform**.
§3's user said the file was *"in downloads something with M5 in the filename"* —
somebody describing a filename from memory does not know its case, and this
whole decision exists to serve that sentence. Implement it explicitly
(`fnmatch.fnmatchcase` against both sides lowered, or an equivalent), never bare
`fnmatch.fnmatch`, and publish the behaviour in the parameter description so the
agent is not guessing.

**Reject both separators, on every platform.** `name_glob` must refuse `/` and
`\` and `..` regardless of the host, or a Windows runbook and a macOS test
disagree about what is refused. `os.sep` alone does not give that.

**Where the depth refusal is.** Decision 5 cites `tools.py:8305`; find it by the
snippet `if children and depth >= max_depth: return refusal(...)`. Note the
refusal helper builds `survey_totals` by re-`stat`ing every accumulated file, so
discovery mode's new non-refusal `truncated` return needs its own shape rather
than a reuse of `refusal()`.

**One thing not to fix here.** `total_bytes` re-`stat`s every file after the
traversal already did. Pre-existing, bounded by `max_files`, and out of scope
unless discovery mode makes it hot.
...
accepted from a runner report. **`start` names the commit the block's branch was actually cut from** — which is the coordination commit recording the assignment, not the commit the baseline was measured on; they differ by documentation only, and both 62c and 62d had to be corrected for naming the latter.
...
| 62d | `design62/inspect-artifacts-discovery` | `fcd7058` (2026-09-01) | Turn 1 `9a9ddeb`, no kills; revision 1 `ddb60a8` (amended) closed four findings. Coordinator commits: `fa2f037` the `scope` type collision, `f03fc75` the junction defect, `ed757cd`/`2846364` gate + self-test, `f37932f`/`2718898` runbook. **Coordinator-run suite: 2756 passed / 99 skipped / 2 warnings, zero failures.** **Evidence, coordinator-watched:** the new file against turn 1's product gives `3 failed / 13 passed`, against revision 1's `2 failed / 15 passed`, and the junction limb against the pre-fix guard fails with the gate's own match list `['control_reached.ilp', 'must_not_be_reached.ilp']`. **Findings:** both new arguments could write a SHA-256 manifest covering 2 of 3 files on disk with no key recording the narrowing (measured) — Decision 5's own *"a partial hash set must never look like a complete manifest"* by a route the bound refusal does not cover; `max_files` had **no description at all**, leaving the examined-not-matched rule and the truncated-negative reading unpublished; the unreadable-directory limb used `chmod(0)` and a literal `"Permission denied"`, unique in the suite, against the repo's own `skipif(os.name == "nt")` convention at three sites; `scope` was a string in discovery and an object in provenance, F4's defect in the response; and **the gate found a bounds defect older than this block** — F8. One self-inflicted defect worth recording: inserting the new helper above `def inspect_artifacts` put it between `@emits_nothing` and its function, so a registered tool silently lost its export decision, caught immediately by `test_every_registered_tool_has_exactly_one_export_decision` | **demo machine, 2 rounds, PASSED.** Round 1: 0 passed / 3 failed / 1 not exercised — one real product defect (junctions followed, F8) and three limbs failed by the *gate* requiring `%USERPROFILE%\Downloads`, absent on that machine. Round 2 after both fixes: **4 passed, 0 failed, 0 not exercised.** Scored from artifacts, not the verdict: sha256 `4cf0c1012276…` is exactly `sha256(b'x'*8)`, so hashing read real bytes; limb A's 6 top-level and limb C's 7 recursive agree with the fixture's shape from independent calls; `truncated=False` with a child directory present confirms *recursive=false is not truncation* on Windows; and **`is_symlink()=False` is identical in both rounds**, so the platform did not change — only the guard did | merged 2026-09-01 `d697f20`, branch deleted local and origin |
...
| after 62c |  **2738 passed / 99 skipped / 2 warnings = 2837** | uv 3.12.14 |
| after 62d — **the live baseline now** | **2756 passed / 99 skipped / 2 warnings = 2855** | uv 3.12.14 |

**`main` now stands at 2756 / 99 / 2.** 62d was measured against 2738 / 99 / 2 and 62c against 2698 / 99 / 2. The pass and skip counts are unchanged
