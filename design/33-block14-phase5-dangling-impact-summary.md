...
| **M5 acquisition budgets were copied from the fictional example, including the exposure limit** (`:790`) — **premise superseded 2026-08-04, see note below the table** | **Yes, M5 values — but not through Phase 5** | **Low.** This is a defect of `microclaw init`'s copy-the-example path (`:350`) plus one deployed M5 config. Phase 5 — no inferred limits, always `reviewed: false` — *replaces* that path rather than inheriting it. | Fix the M5 config as its own rig-config review item. Optionally have Phase 5 detect unchanged example values, but that is net-new hardening, not dangling work Phase 5 must absorb. **Both landed in Block 5 (2026-08-04): `init` no longer copies the example by default, and `check-config` detects unchanged example values. The M5 config edit itself is still open.** |
...
- require human-entered safety bounds. Phase 5 structurally cannot emit example
  ...
  path, not a Phase 5 obligation. **That check landed in Block 5 (2026-08-04) as
  `check-config`'s non-blocking `example_limits` diagnostic. Note what it cannot
  do: it answers "still the example's values?", never "are these values sane."
  M5 passes it cleanly while carrying a ~317,000-year acquisition duration cap;**
