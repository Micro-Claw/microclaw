> **ARCHIVED — NOT PART OF THE NUMBERED `design/` SEQUENCE.**
>
> This documents the jPypeMM + AcqEngJ backend port, which was built on the
> branch `port-to-jpype-acqj` (tip `e9b9234`), **reverted on `main`, and never
> shipped**. `main` runs the pycro-manager ZMQ backend — see `CLAUDE.md`.
>
> Recovered into `design/` on 2026-08-24, just before that branch was deleted.
> The implementation is gone and the codebase has diverged far enough that the
> port would be redesigned from scratch rather than resumed. This file is kept
> for its rationale alone, and is deliberately unnumbered so it never reads as
