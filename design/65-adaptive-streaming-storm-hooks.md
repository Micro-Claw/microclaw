...
| 65c | `design65/adaptive-timelapse-route` | `cad15b9` (2026-08-31) | `96b3b72` + `01beb5f` (runner turn 1, killed by a Codex usage limit after its commits and before it ran the suite or reported — coordinator recorded that in `c0f01af`) + `bb3e970` + `89af6a9` (revision round 1, six findings and four test defects) + coordinator `912f5e3`. Suite **2645 passed / 99 skipped / 3 warnings**, coordinator-run on a clean tree, baseline + 23. Round 1's confirmed defect: the successor replaced the seed's whole `axes` dict, so a channelled run's frames 1..N dropped the `channel` axis — design/43's dense-axis failure one axis over. Its test missed it because the fixture supplied the successor; it now drives `run_timelapse`'s own closure with a channel set. Round 1 also mocked the whole runner out of design row 13, which now reaches `_acquire_with_hooks` and observes the 24 h fallback, `frames_planned`, and the reservation cap over 37 real frames + `9834b93`/`723a9d3`/`b3096d7` (gate) + `0d0a8ec` (gate review round 1). Suite **2646 passed / 99 skipped / 3 warnings**, coordinator-run, baseline + 24. The gate's review found a blocker the implementation did not: both shipped skills still said an image-driven conditional stop is refused and that no adaptive STORM hook should be offered, and every driven limb asks an agent to do exactly that — so design/65's plan to correct them *after* 65c merges was wrong, because the gate session is an agent reading them. Folded in, scoped, with the still-true statements kept and qualified to the fixed route | **M2 required** — §"Rig gate": three operator-judged limbs (successor dispatch and early stop with no hardware capability granted; an image-derived write to a non-dosing property; a short authorized `Duration0` run on a frame-index predicate). Everything that only computes goes in one script, run against `design/55-gate-probe-selftest.py`'s bridge-shaped fake before it ships |. Computed gate `design/65-block65c-gate.py` verified by the coordinator on both trees: **6/6 PASS** on the branch, **A–E FAIL with existing-route control F PASS** on pre-65c `main`. Runbook `design/65-block65c-m5-runbook.md`, pinned by `git merge-base --is-ancestor 912f5e3 HEAD`. **M2 ran 2026-08-31 and was scored in `design/65-block65c-m2-scoring.md`.** Limbs 2 and 5 PASS; limbs 3 and 4 NOT EXERCISED (3 was unrunnable by construction — it named a stage-bounded device, which design/49 refuses; 4 trailed off with no verdict). The computed gate's 3/6 was **the instrument**: three limbs read back an export path the product had resolved elsewhere, which only coincides when `--out` is absolute, and the selftest only ever passed absolute. Reproduced off-rig and fixed; all four tree×out-shape combinations now discriminate. **The cadence measurement design/65 refused to assume: 99 gaps at 50 ms exposure, min 0.219 / mean 0.2495 / max 0.344 s — ~200 ms of software per frame, a 5× cadence cost, n=1 from M2, unattributed.** **M2 access ended after round 2.** The export limbs closed on M2 round 2 (6/6); what remains moved to **M5**, which is arguably the better rig for it — the FPGA activation-pulse duration limb 4 was never exercised in either round is htSMLM's own route. `design/65-block65c-m5-runbook.md` carries the M2 evidence forward as already-established rows and sends only the open limbs to M5: limb 3, limb 4, a bounded-stage refusal control for the unevidenced C1 authorization change, and a three-way cadence attribution set (fixed / adaptive-with-analysis / adaptive-without-analysis) on one field. **M5 ran 2026-09-01: limb 3 PASS with restoration, the C1 control fired on an excluded pair, and limb 4 finally ran its mechanism** (writes at times 0-2 with read-back, StopAcquisition at time 3) **and failed only at restoration** — `restore: entry` was charged against `max_writes`, so a run that promised restoration could not keep it and the operator restored the rig by hand. The fixed path had validated `max_writes >= planned + 1` up front; the adaptive path cannot know the count up front, so nothing reserved the write. Restoration is now never charged, on both envelope kinds, and may write the exact recorded entry value even when it falls outside the *proposal* bounds — the rig's own limits still apply. **The M5 cadence attribution supersedes M2's headline number**: fixed 0.0502 s, adaptive-with-analysis 0.0912 s, adaptive-without-analysis 0.0909 s at 50 ms — ~41 ms of dispatch per frame, analysis free, ~1.8x not 5x. M5 also exposed that **no read-only way existed to ask a pair's authorization classification**, so an agent had to spend a write to learn a policy; that is now folded into `get_device_property_info`. Limb 4 owes one short re-run. **M5 round 2: limb 4 PASS** — three hook writes against `max_writes=3` plus an uncharged restoration write to entry, requested == achieved on all four, `restored: true`, `stop_reason: hook_stop`, 4 of 8 frames, and a 3481-line standalone export carrying the rule. Every gate limb now has microscope evidence | `e984a97` merged 2026-09-01, branch deleted local and origin; gate scoring in `design/65-block65c-m2-scoring.md`, runbook `design/65-block65c-m5-runbook.md`, computed gate and both-tree selftest in `design/65-block65c-gate*.py` |
