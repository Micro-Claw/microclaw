...
| 9 | `design29/stage-coordinate-mosaic` | `fecb93b` | through `bd63d37` (3 blockers + 2 + 2 coordinator rounds); 1092/99/3 | **PENDING — rig gate RUN 2026-07-29 (`b9grid_2`, `b9yline_1`). R1/R2/R3 pass; first non-sentinel per-image affine ever recorded. R3b: no objective key even with `Res1` live, so the acquisition-recorded branch is unreachable on M2 — finding, not defect. R4: **Y column corroborated at last** (cross-axis residual +0.40 px, sd 0.05), but stage-X carries a systematic +11.00 px (sd 1.27, 10.0%) and fails R4's single-digit criterion — axis-selective, so stage/scale, not transform. **R5 unanswerable on beads: global orientation still unchecked, and that is what blocks the merge.** Follow-up is R6.** Review caught a mosaic that rendered solid tiles as stripes | | |
