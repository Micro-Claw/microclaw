...
| 80c | — | **not started. Its first step is a probe, not a runner**: `design/80-block80c-oughtafocus-probe.py` (pushed `d08ab45`, verified against bridge-shaped fakes on all four branches) answers the one question the design defers — whether the returned `AutofocusMethod` exposes `get_property_names()`/`get_property_value()` over the bridge. Needs a Micro-Manager with OughtaFocus and the bridge on; no dose, no stage motion. **Write no checklist until its answer is folded into §80c**, because that answer decides whether the emitted script can print the plugin's settings or must declare them an external precondition. | | | |
