...
| 77b | `design77/acquisition-order` | `7c33284` (2026-09-06), worktree `../microclaw-77b`, baseline **2868 passed / 99 skipped / 4 warnings** measured *in the runner's own venv* — the primary checkout reports 2 warnings, and handing over the wrong number is how a runner gets a finding it cannot meet. | `20c997c` + `05eaf32` + `16dfae2` + `c7ebdf5`. **Both Codex turns were killed, neither produced a report.** The start turn died to *coordinator-machine memory pressure* after committing `20c997c`; its `session-id` was never written and was recovered from `initial.events.jsonl`'s `thread_id` using the wrapper's own extraction expression. The revision turn died to a *provider usage limit* mid-edit (reset 13:51), leaving uncommitted work that **did not parse** — `timing={}` inserted ahead of positional arguments. That is `05eaf32`, committed as UNREVIEWED per the workflow, with `16dfae2` repairing the one line so the suite could collect.
**Six review findings, one confirmed by probe rather than by reading.** F1: a `StageMoveError` on field B of a spaced hooked run was flattened to a string at `tools.py:7420` and field A's finished dataset was dropped from the report entirely — probed through `execute_tool`, which returned `['error', 'position_list_rollback']` while `.../A/A` sat on disk with three accounted frames. The hookless loop re-raises deliberately at `:7493`; the new split path did not. The runner's own boundary test covered `AcquisitionUnterminated` only. F2: the schema had lost the *"doing both repeats every exposure"* dose warning. F3: the mosaic restriction was stated on only one of the two routes that now write per-position datasets. F4/F5/F6: an unexplained EMU trigger preflight, `frames_acquired` changing meaning on a path that need not change, and two `timing` shapes from one tool.
**The killed revision then shipped a worse defect than the one it fixed**, caught only by the coordinator re-running the suite: it declared `positions_completed` as an instance attribute in `StageMoveError.__init__`, and `XYStageMoveError` overrides `__init__` without calling `super()` — so **`execute_tool`'s own error handler raised `AttributeError` on every XY move failure**, four failures in design/68's boundary test. `c7ebdf5` moves it to a class attribute, adds a regression test over `StageMoveError.__subclasses__()`, and finishes F2/F3. Watched it fail with only `controller.py` reverted; the first attempt failed on a wrong fixture key (`KeyError: 'x_tolerance_um'`) and was re-watched until it failed for the stated reason.
**Watch-it-fail and mutation were both reproduced by the coordinator, not taken from the runner.** All three round-1 tests fail on the pre-fix tree for their stated reasons. Four mutants — flipped `ptcz`/`tpcz`, `split=False` in the tool, `order` dropped from the emitter, `split=False` in the emitter — each kill their intended test, no survivors. Coordinator suite **2897 passed / 99 skipped / 0 failed**, reconciling exactly as 2868 + 22 + 6 + 1. The four failures the runner reported as environmental are genuinely sandbox artifacts and pass outside it.
**One design-doc error corrected before handoff**: checklist item 6 claimed the tool routes to `_emit_adaptive`/`_emit_acquisition`; it is `@emits(_emit_multiposition)`, which branches internally, with `_emit_tile` and `_emit_multiposition_with_autofocus` forwarding into it. | **NOT RUN.** Runbook `design/77-block77b-gate.md`, program `design/77-block77b-demo-gate.py`, selftest `design/77-block77b-gate-selftest.py`. Demo machine, ~6 min, ~24 demo-camera exposures, one 20 µm XY move returned. Eight limbs, only what a fake cannot answer; limb E is the control. Selftest is bridge shaped (`size()`/`get(i)`, `__iter__` raises), six cases, four deliberate failures, run on **both** trees: 6/6 here, and on a pre-77b tree E FAILs with seven NOT EXERCISED. It found two defects in the gate and one in its own fake, none in the product. **Limb D is the limb that can change the product**: it enumerates the camera's per-image metadata keys and either names an exposure-timestamp key for D3 or confirms the shipped `null` is correct here. | not merged |
