...
| 77b | `design77/acquisition-order` | `7c33284` (2026-09-06), worktree `../microclaw-77b`, baseline **2868 passed / 99 skipped / 4 warnings** measured *in the runner's own venv* — the primary checkout reports 2 warnings, and handing over the wrong number is how a runner gets a finding it cannot meet. | `20c997c` + `05eaf32` + `16dfae2` + `c7ebdf5` + `81f496b` + `b4471ba`. **Three of four Codex turns were killed; two produced no report.** The start turn died to *coordinator-machine memory pressure* after committing `20c997c`; its `session-id` was never written and was recovered from `initial.events.jsonl`'s `thread_id` using the wrapper's own extraction expression. The first revision died to a *provider usage limit* mid-edit, leaving uncommitted work that **did not parse** — `timing={}` ahead of positional arguments — committed as UNREVIEWED (`05eaf32`) per the workflow, with `16dfae2` repairing the one line. **Six review findings, one confirmed by probe:** a `StageMoveError` on field B of a spaced hooked run was flattened to a string at `tools.py:7420` and field A's finished dataset dropped entirely — probed through `execute_tool`, which returned `['error', 'position_list_rollback']` while `.../A/A` sat on disk with three accounted frames. Plus a deleted dose warning, the mosaic restriction stated on only one of two routes, an unexplained EMU trigger preflight, `frames_acquired` changing meaning, and two `timing` shapes. **The killed revision then shipped a worse defect than the one it fixed**, caught only by re-running the suite: `positions_completed` as an instance attribute in `StageMoveError.__init__`, while `XYStageMoveError` overrides `__init__` without `super()` — so **`execute_tool`'s own error handler raised `AttributeError` on every XY move failure**, four failures in design/68's boundary test. `c7ebdf5` moves it to a class attribute with a regression test over `StageMoveError.__subclasses__()`. **Item 5 (`81f496b` + `b4471ba`) landed after the gate**, because it was conditional on the key limb D found. Two further findings there, both probed: a hooked Z-stack reported *"spacing is ambiguous"* for a run with no time axis, and interleaved per-field spacing is a **revisit interval** with nothing saying so — 0.02 s reported for a camera really firing every 10 ms. **Watch-it-fail and mutation were reproduced by the coordinator throughout, never taken from a runner's table**: eleven mutants across three rounds, no survivors. Coordinator suite **2908 passed / 99 skipped / 0 failed**, reconciling as 2868 + 22 + 6 + 1 + 7 + 4. **One design-doc error corrected before handoff**: checklist item 6 claimed the tool routes to `_emit_adaptive`/`_emit_acquisition`; it is `@emits(_emit_multiposition)`, which branches internally, with `_emit_tile` and `_emit_multiposition_with_autofocus` forwarding into it. | **RUN 2026-09-06, Windows demo machine: 7/8 PASS.** Limbs A/B: real AcqEngJ executes a `ptcz` position-axis list in submitted order (`gateA gateA gateA gateB gateB gateB`), and `time_then_position` interleaves. **Limb C is the strongest artifact**: field B's acquisition began 0.601 s after field A's last frame, then held 2.03 / 2.002 s gaps, spans 4.061 s and 4.033 s — under one shared clock B's deadlines (0, 2, 4 s) would all have been past and B would have burst. Limb F ran the exported script standalone against the bridge and got the same order. The control arm on `main` failed at E with seven NOT EXERCISED. Datasets agree with the hook logs and each split dataset keeps its own `position` axis. **The one FAIL was the gate's, not the product's, and cost no second trip:** limb G called `build_stage_coordinate_mosaic` with no `axis_selection` on a dataset carrying `time=[0,1,2]` and the tool refused an ambiguous selection, correctly. Settled off-rig from the returned dataset — with `{'time': 0}` it mosaics both tiles, `coverage_fraction` 1.0. **The selftest could not have caught it**: no fake produces a real NDTiff, so the healthy case could only assert G does *not* pass. A limb the selftest can check only negatively is the one to read hardest on the first real run. **Limb D was the point of the trip**, returning `ElapsedTime-ms`, `TimeReceivedByCore` and `Time` — unblocking item 5, which was implemented afterwards on this branch and verified off-rig against the returned dataset's own metadata, so it needed no second trip. `R99` records that the *live* post-acquisition read has still never run on hardware. | `6381902` merged 2026-09-06; branch deleted locally and on `origin`, worktree removed, notes in `design/prompts.md`. Post-merge design gate done on the branch before merge: §"77b — what shipped" supersedes the commissioned checklist, the gate doc's limb D renamed off "exposure-timestamp", and rows `R98`/`R99` opened. **77c is untouched by this block.** |
| 77c | — | not started; follows 77b, which is now merged | | | |
