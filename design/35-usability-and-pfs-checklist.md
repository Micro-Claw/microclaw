...
- **RESOLVED 2026-08-12 (`78ac1b7`) — a Windows-only socket race in
  ...
  regression, not of the fix.** The Windows proof is booked as Step 0 of
  43n's demo pre-gate, which loops the single test 100 times — one clean run
  cannot settle a race that skipped a round on the same machine. Original
  finding follows.
