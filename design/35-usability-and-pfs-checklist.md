...
- **43j is implemented and pushed** — branch
  `design43/hooks-and-timelapse-observation`, two implementation rounds plus one
  coordinator fix, runbook `c288746` pinned `f36ea89`. **Awaiting the demo
  machine; only its Step 7 needs M5 or M2.** **43k is last and stays
  design-only**; its dependency is satisfied — 43h and 43i have both run on a rig.
- **Suite baseline: 1797 passed / 99 skipped / 3 warnings, 1896 collected** on
  macOS at `f36ea89` (1790 / 99 / 1889 at `448c4a4`, before the block); Windows
  expectation 1780 + 116 = 1896.
