...
| 66 | Register promotion | design/64 and 64c merged | `design66/stage-move-response` | `fa9b3cf` | **runner assigned after coordinator setup commit** | **required — M2 relative-response move + standalone selected-call export + safe genuine non-response control; demo instant-arrival regression.** See design/66 §"Rig gate" | | **in flight 2026-08-30** — design/66 owns the response-versus-accuracy decision; must add an owned cross-plane non-response row before close |
