...
- **Block 46's first demo gate ran 2026-08-12 and its Part A was `NOT TESTED`,
  ...
  `mmc.setExposure(20)` repaints the Exposure box the moment Run is pressed, so
  every row observed a repaint that would have happened anyway. Self-confirming,
  and the coordinator's own §3 caused it by assuming a raw core write leaves the
  GUI stale. The runbook now decides between "MM repaints on any core write, so
  the eleven calls are a no-op" and "the Script Panel repaints after running a
  script while a ZMQ write does not" with **an A/B against `main`**, and stops
  Part A outright if the machine self-repaints. **Do not record 46 as gated
  until that A/B comes back.**
...
- **Two blocks are now awaiting the same machine**: 46's §3 A/B and 47's gate.
  They touch `set_roi`/`clear_roi` in common, so **whichever merges second merges
  `main` first.**
...
| 46 | Platform | none — assigned while Track C is parked | `design46/gui-refresh-sweep` | `d938098` | `c68405a` + runbook `1f1bbf4` + `364ffcc` (review round 1 returned, four findings); runbook pinned `c68405a`, which survives the round-2 commit **because the pin is `merge-base --is-ancestor` on the implementation, not the tip**. **Eleven `ctrl.refresh_gui()` calls, no new layer.** Round 1 accepted the implementation and returned four findings, **three of them in the runbook** — the same distribution as block 45. (1) **Step 7c could silently test 7b's call site**: it targets `_acquire_positions_with_hook` (`tools.py:4889`), reachable only via `run_multiposition_acquisition`, but asked for "a one-position hooked acquisition, one frame", which routes to `run_timelapse(n_frames=1, hook_strategy=…)` — the route **the codebase itself recommends at `tools.py:4258`** — whose write is `:2738`. The operator would have watched the box repaint and recorded PASS for the wrong line. Now names the tool with exact `protocol_params` and makes wrong routing `NOT TESTED`. (2) **Step 7d named a tool that does not exist**: `_acquire_survey_with_detector` has exactly one caller (`run_adaptive_survey:5515`), so "run a one-position survey with the built-in detector" left the agent to improvise; a step that passes on a correct guess is not evidence. Now names `run_adaptive_survey` with `acquire_on_hit` omitted, and **states that 7d and 7e are two call sites inside one tool** (`:5234` search-phase, `:5544` acquire-phase), which the runbook had left the operator to infer. (3) **Step 7a commanded an absolute Z of 0 µm.** `run_zstack`'s Z arguments go straight to `guard.check_z` (`:2582`); block 45's M5 evidence puts that focus device near **50.6 µm**, and M5 still carries an example-derived `stage.z_min`, so the step was either a refusal the operator would improvise around or an unnecessary ~50 µm objective move — on a step whose only job is watching an exposure box repaint. Now reads the current focus position first and uses `CURRENT_Z → CURRENT_Z + 2`, with an unmeasured absolute Z or a safety refusal recorded `NOT TESTED`. (4) **Seven of eleven call sites had no test asserting the refresh** — the precise state that created this block, since 43b added four and nobody noticed the rest; all seven now assert, and the `run_adaptive_survey` case was verified to cover `:5544` rather than counting `_set_channel_for_composite`'s repaint, which the test monkeypatches away. **Two prompt premises were contradicted by the implementer and both were verified and accepted**: none of the five eventless exposure writes is per-tile (each runs once at an acquisition or phase boundary, both helpers taking the full position list), which dissolves the coordinator's bridge-cost objection and makes "refresh every row" correct rather than assumed; and refreshing every row needs **eleven** call sites, not the six the assignment stated. Suite 1826 / 99 / 3 = **1925 collected**, coordinator-re-run in the worktree at both rounds rather than accepted | | | |
...
# Block 46 — [ ] the GUI-refresh sweep: every writer that leaves Micro-Manager stale

Branch: `design46/gui-refresh-sweep`
