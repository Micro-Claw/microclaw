...
| 9b | Features | 9a merged; **rescoped 2026-08-25, no longer blocked on a fixture** | to be created | | | **The verdict is struck by operator ruling 2026-08-25** — whether ilastik beats the classical floor is a research question about ilastik, not a product question about Microclaw, and the floor spike, the adjudicated fixture, the M2 survey and the verdict go with it. What remains is one shipped constant never chosen from data, `coverage_floor` (default 0.01). **It wants M5, not M2 — the reverse of the earlier note** — because the only classifier that works on real fields is the operator's M5-retrained project, and M2 was needed only to keep the struck verdict free of a domain shift. **It may need no rig at all**: 108 fields from 9a's M5 session are already in the archive. Superseded text follows. ~~Split out of block 9 on 2026-08-24~~: building the adapter and deciding whether it beats the classical floor have different gates, and only the second needs adjudicated fields. Carries the floor spike (not on `main` — `ClassicalDescriptor`/`FewShotProbe`/`detectors.py` are stubs, the working code is `design/26-roi-detection-spike.py`), the `mostly` threshold and `mito_coverage` floor, the apo-vs-healthy channel confirmation that a synthetic frame cannot make, the M2 survey, and the verdict. **The session confound goes in the verdict whatever the result.** A negative result is a legitimate outcome | | |
...
## 9b. Design/26 Run B — the coverage floor

Branch: to be created.

**Rescoped by operator ruling, 2026-08-25. The verdict is struck.** This block
used to ask whether ilastik beats the classical floor. That is a research question
about ilastik, not a product question about Microclaw — *"the user owns the
session"*, and a microscopist who wants ilastik is entitled to use it. The floor
spike, the adjudicated-fields fixture, the M2 survey and the verdict itself all go
with the premise; design/26 F5 is corrected in place rather than annotated.

**What survives is one shipped constant that has never been chosen from data**:
`coverage_floor`, default `0.01`. Below it the adapter reports `unresolved`
instead of a ratio, because a field with almost no signal produces a wildly
unstable one.

**It needs M5, not M2 — the opposite of this block's earlier note.** M2 was only
ever required to keep the *verdict* free of a domain shift. With no verdict, what
the floor needs is real fields scored by a classifier that actually works on them,
and the working classifier is the operator's M5-retrained project
(`260825_Mito-classify_M5.ilp`). The M2-trained project does not transfer: on a
field the operator called apoptotic it assigns **twice as much healthy as
apoptotic** probability (BG 0.969 / apo 0.010 / healthy 0.021).

**And it may need no rig time at all.** Block 9a's M5 session left **108 fields
already acquired** in the evidence archive (`9a-sarah-m5`: surveys of 25, 25, 49
and 9), scored at the time with the M2 project. Re-scoring those with the
retrained project is a computation over data already on disk.

- [ ] Re-score the existing M5 surveys with `260825_Mito-classify_M5.ilp` and plot
      the coverage distribution. Both classes of field are needed: ones with cells
      and ones with none. Note that this project trained only `BG` and `apo_mito`,
      so score `apo_mito` against `BG`.
- [ ] Choose the floor from where those two populations separate, and record the
      measurement rather than the number alone. Measured so far: **0.085–0.159**
      on apoptotic fields with the retrained project, **0.106** on demo frames,
      and **0.0005–0.008** with the non-transferring M2 project — the last being
      the gate correctly reporting a classifier that does not work here.
- [ ] If the two populations do not separate on coverage, say so and leave the
      default: a validity gate that cannot discriminate is worse than an
      acknowledged guess.
- [ ] Only if the archive cannot answer it: a short M5 survey including
      deliberately empty fields.

Post-merge design gate:

- [ ] Record the chosen floor and its evidence in `design/26-implementation.md`,
      including the M2-to-M5 transfer measurement, which is the block's most
      reusable result.
