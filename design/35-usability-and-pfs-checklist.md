...
- **43j is in flight, demo round 1 PASSED all seven demo steps** — branch
  `design43/hooks-and-timelapse-observation`, two implementation rounds plus two
  coordinator fixes, runbook `20214b5` pinned `673e721`. **Round 2 owes Step 6
  again (one criterion was short) and Step 7, the EMU `laser_slot` limb, which
  is the only part of this gate that needs M5 or M2.** **43k is last and stays
  design-only**; its dependency is satisfied — 43h and 43i have both run on a rig.
- **Suite baseline: 1799 passed / 99 skipped / 3 warnings, 1898 collected** on
  macOS at `673e721` (1790 / 99 / 1889 at `448c4a4`, before the block); Windows
  expectation 1782 + 116 = 1898.
