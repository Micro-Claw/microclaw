...
| 64b | Register promotion | design/64 merged | `design64/kb-lost-update` | `5544d90` | `16ff7f1` | **none needed** — the defect is reproducible with two processes; `design/64-kb-lost-update-probe.py` lost **40 of 80** disjoint keys pre-fix, 0 of 80 after | *pending* | design/64 §"The lost-update race, closed"; register row closed. **Step-2 deviation, recorded not hidden:** implemented by the coordinator inline before the coordinator role was assigned, not delegated to a runner in its own worktree. A runner review of the diff was commissioned after the fact, which is not the same thing — same shape as 64a, and the second time in two blocks. |
