...
### State at the 2026-08-11 assignment of block 43f — read this before assigning anything
...
| 43f | Nestor | 43a merged (its prompt names the key this creates) | `design43/rig-profile` | `1a968be` | | **required** — the interview is a conversation with a human, and no offline test can show that it asked well or that a stored fact reached the operator's own question. Rig with a deliberate crop and camera-triggered lasers preferred (M5 or M2), because that is the profile whose two facts 43a's prompt text already reads | | |
...
- [ ] **`CATEGORIES` is not the only list of categories.** Three tool schemas
...
- [ ] **`save_knowledge` is confirm-gated `kind="knowledge"`, and 43c made
...
- [ ] **`format_for_prompt` is not category-neutral.** It announces the block as
...
- [ ] **`rig/calibration` is not `devices/affine_*`.** `calibration.py` writes
...
- [ ] **One assembly site.** `_system_blocks` (`agent.py:266`) is reached by both
...
- [ ] **Not in `first_launch`.** `first_launch.interview()` is a terminal wizard
