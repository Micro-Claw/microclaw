...
| 64c | Register promotion | design/64 merged; 64b merged | `design67/tol-px-floor` | `0fc97b7` | *assigned* | **none — no rig gate.** Every mechanism is a computation over values the loop already holds; the M2 data that sized it is already in `gate64-m2`/`gate64b-m2`. Scoring the new hint on a rig needs a session that *fails* to converge — dose spent to observe a string | | design/67 is the design |
