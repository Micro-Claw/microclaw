...
| 64c | Register promotion | design/64 merged; 64b merged | `design67/tol-px-floor` | `0fc97b7` | `f4c31f6` — **LANDED BUT UNREVIEWED. Step 3 has not run.** The runner reported suite 2554 / 99 / 3 and per-test failure evidence; that is a handoff, not evidence. Read the diff and rerun the suite before believing any of it | **none — no rig gate.** Every mechanism is a computation over values the loop already holds; the M2 data that sized it is already in `gate64-m2`/`gate64b-m2`. Scoring the new hint on a rig needs a session that *fails* to converge — dose spent to observe a string | | **PARKED 2026-08-30 by operator decision** — design/66 is being reworked offline first; resume 64c after it. Branch `design67/tol-px-floor` is pushed and carries both commits; worktree `../microclaw-64c` left in place (`git worktree add ../microclaw-64c design67/tol-px-floor` recreates it). design/67 is the design |
...
| `tol_px` is not checked against the stage's step size | design/64, M2 2026-08-30 | **Block 64c** (design/67, assigned 2026-08-30) — M2 quantizes ~0.8 µm (design/29), which at 0.127 µm/px is a **6.3 px floor against `center_feature`'s 5.0 px default**, so convergence there is partly luck: limb F needed one correction (72.3 → 2.5 px) and limb F2 needed three (47.0 → 3.9). The non-convergence `hint` blames a stale calibration and never mentions the stage. Smallest of the three, and the M2 data to size it already exists in `gate64-m2`/`gate64b-m2`. |
