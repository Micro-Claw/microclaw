...
| 42b | Read side | 42a's answers | `design42/open-artifact` | `4d6a426` | `0dd3629` (dir spike) + `410846e` + `e02955f` + `ebebe45` + `ed9c78a`; review round 1 `867f3af`; runbook `a038c95`/`838f00d`/`b06fbac` pinned `867f3af`; findings `c60d33f`; fix round `229423d` (runner) + review round 2 `f574643`; **redesign `a97620b`** (open the dataset's TIFFs, MM reader deleted); runbook re-pinned `a97620b`; round-3 fix `5d09b7b` |  **demo 2026-08-09 at `bc93f76` — FILE branch PASS (G1/G2, no thumbnail), DIRECTORY branch FAIL**: G0 D2 ERROR (`NDTiffAdapter` casts axis values to Integer) and G4a wedged the bridge ~5 min. Findings F1–F3 in `design/42-block42b-gate-findings.md`; demo 2026-08-09 round 3 **G1+G2+G3+G4a+G4b+G4c ALL PASS**, no stalls, one image block in the session and only on the `analyze=true` turn (`41b-open-artifact-demo-round3`). `stitch_test_1` — which MM's reader could never read (**F1**) — opens with all 6 tiles; **F4** (no channel axis) is why that reader is deleted. Round-3 finding fixed in `5d09b7b`: a non-dataset directory now measures the files it actually opened. **G0 retired.** Multi-channel axis structure is a **known, tabled limitation**. **M5 still owed** — read-side block, demo-gated by design — the demo run does not discharge it | | `0819790` | **pending** — design/42 reconciled in-branch; carried-forward rows still to tick | 9 | Features | operator intake | `design26/generated-adapter-run-b` | | | required | | |
