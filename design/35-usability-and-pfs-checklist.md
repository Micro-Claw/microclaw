...
# Block 45 — repair the saved-hook path — **assign first, before Track C**

Branch: `design45/saved-hook-repair` — **not created; assign first**
...
- [ ] **Refuse at save time what `_resolve_hook` will refuse at run time.**
...
- [ ] Ask the agent for a hook in ordinary language, and confirm a hook violating
      the contract is **refused at save time** with the reason and the fix, rather
      than saved and recommended.
- [ ] Save a valid hook, attach it, run a one-tile survey, and confirm it resolves.
- [ ] Export a session containing an observation-only hooked acquisition and
