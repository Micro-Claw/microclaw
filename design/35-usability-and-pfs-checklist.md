...
| 47 | Platform | none — promoted from the open register by operator decision | `design47/camera-roi-capability` | `d5d3470` | | required: the demo machine carries the whole block (ROI is camera geometry). **Block 46's A2 is the same code path and cannot close it** | | |
