...
- **43n is implemented, reviewed through one returned round, and pushed
  2026-08-12 — it is awaiting M5.** Runbook `design/44-block43n-rig-gate.md` is on
  the branch, pinned `db8baf5`. The ledger row carries the four round-1 findings
  and what round 2 did with them. **The block's remaining precondition is the
  Windows socket race**, below: the runbook states an expected warning count and
  that race intermittently adds a fourth.
