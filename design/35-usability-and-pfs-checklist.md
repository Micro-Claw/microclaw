...
- **47 is implemented, reviewed through one returned round, and pushed — it is
  awaiting the demo machine.** Runbook `design/47-block47-rig-gate.md` is on the
  branch, pinned `9018188`. **One of round 1's two blocking findings was the
  coordinator's own checklist item**, which required `clear_roi` to go through a
  guard that has nothing to validate; it is corrected at `7747279` and recorded
  as an error, not reworded. The other made `set_roi` shrink-only by treating the
  current ROI as a bound.
- **Block 47's open question is a measurement the rig owes, not a decision.**
  The full-frame extent is not portably knowable (`getImageWidth`/`Height`
  describe the image buffer), so microclaw now guards only integrality and
  positivity and lets the adapter refuse its own sensor geometry. Whether an
  adapter accepts a reposition outside the current crop is what the gate's
  opposite-corner step measures. **Either outcome is evidence**; a microclaw
  `SafetyViolation` there is a gate failure.
- **Block 47 is the only block awaiting a rig.** 46 is dropped, so the
  `set_roi`/`clear_roi` collision that would have forced a merge order is gone —
  47 merges into `main` unobstructed.
...
# Block 47 — [ ] give camera ROI a typed capability, and stop misdirecting the operator

Branch: `design47/camera-roi-capability`
...
| Camera ROI has no typed capability | design/33 `:708`, `:831` | **Block 4** emits exclusion; capability itself **now Block 47** — promoted 2026-08-12 after an operator hit it on the demo machine during block 46's gate. The misleading "the safety config would have to change first" refusal is folded into 47, not tracked separately |
