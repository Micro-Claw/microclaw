...
| The knowledge base has a lost-update race between concurrent launches | design/64, 2026-08-30 | **(no block)** — `knowledge_manager.save_entry` loads the whole document, edits and rewrites it. design/64 made the torn-read window atomic (`_write_knowledge`) but **not** the read-modify-write, so two launches can still drop each other's unrelated edits. Predates design/64, which only made it more frequent by letting `find_features`/`center_feature` adopt MM's affine on a read path. **Needs no rig** — reproducible with two processes. |
