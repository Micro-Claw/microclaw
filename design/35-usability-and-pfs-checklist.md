...
| `tol_px` is not checked against the stage's step size | design/64, M2 2026-08-30 | **(no block)** — M2 quantizes ~0.8 µm (design/29), which at 0.127 µm/px is a **6.3 px floor against `center_feature`'s 5.0 px default**, so convergence there is partly luck: limb F needed one correction (72.3 → 2.5 px) and limb F2 needed three (47.0 → 3.9). The non-convergence `hint` blames a stale calibration and never mentions the stage. Smallest of the three, and the M2 data to size it already exists in `gate64-m2`/`gate64b-m2`. |
