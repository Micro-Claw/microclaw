...
| 46 | Platform | none — assigned while Track C is parked | `design46/gui-refresh-sweep` | `d938098` | | required: the measured `set_exposure` row, plus **one glance per inspection row**, each naming the rig it ran on. Demo carries most of it; do not claim demo sufficiency without saying so | | |
