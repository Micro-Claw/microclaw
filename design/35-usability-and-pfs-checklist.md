...
- **43j is in flight and one criterion from done** — branch
  `design43/hooks-and-timelapse-observation`, two implementation rounds plus
  three coordinator fixes, runbook `351bea6` pinned `25510bb`. Every step has
  passed except **Step 7b**, which the M5 session did not reach; round 3 runs
  Step 0 and 7b only, on M5 or M2. **43k is last and stays design-only**; its
...
- **Suite baseline: 1800 passed / 99 skipped / 3 warnings, 1899 collected** on
  macOS at `25510bb` (1790 / 99 / 1889 at `448c4a4`, before the block); Windows
  expectation 1783 + 116 = 1899.
...
- [ ] F12 — `run_adaptive_timelapse` is **deleted**, not deprecated: its tool
      ...
      compatibility.
- [ ] Symmetry — `run_zstack` gains the same five arguments and
...
- [ ] **The exposure trap, shared by both folds.** `_emit_adaptive`'s
