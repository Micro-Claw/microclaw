...
- **Block 46 is DROPPED unmerged (operator ruling, 2026-08-12). Its branch is
  deleted locally and on `origin`, and no code from it is on `main`.** Do not
  re-propose a GUI-refresh sweep. The reasoning and the recurrence investigation
  are in the register entry "The GUI stops tracking after an *exposure* write";
  read it before touching this area.
- **The `refresh_gui()` calls already on `main` are block 43b's EMU fix. Leave
  them alone.** Six sites (`tools.py:2081`, `:2169`, `:6893`, `:7049`;
  `authorization.py:1731`, `:1752`) exist because EMU sometimes will not repaint
  without an explicit call; that fix was measured and works, and `:7049` is the
  laser-property case specifically. They are **not** an instance of the dropped
  block's claim and must not be generalised or cleaned up.
- **Micro-Manager repaints itself on property changes** — it publishes
  `PropertiesChangedEvent`. Measured on the demo machine against `main`, whose
  `set_exposure` contains no repaint at all and which updated the box anyway.
  **Any future "the GUI is stale" finding starts from that**, not from the
  assumption that microclaw owes a repaint after every write.
- **Why it was dropped, in one line each:** the demo A/B showed the machine
  self-repaints; the M5 sighting was one unreproduced session; and the design
  hung every repaint on a *write* while the M5 session made no exposure write, so
  none of the eleven calls would have fired. The last of those needed no rig and
  was derivable at assignment from a paragraph already in this file.
...
- **The process lesson from 46, which outlives it: a gate that cannot fail proves
  nothing, and nine PASS rows can be worth zero.** Its first demo run passed nine
  rows that were never evidence — the runbook manufactured staleness with the
  Script Panel, which repaints on Run, so every row observed a repaint that would
  have happened anyway. The coordinator's own §3 caused it by assuming a raw core
  write leaves the GUI stale. Replacing that with **an A/B against `main`**
  refuted the entire block in one prompt. Before booking rig time, ask what
  result would falsify the block, and make sure the runbook can produce it.
- **Block 47 is assigned, promoted from the open register.** Camera ROI has had
  no typed capability since `2599869` (2026-07-22) and is refused on every
  map-carrying rig; it worked from `55e9eb8` (2026-06-20) until then. Found by
  the operator on the demo machine during 46's A2, which is the same code path
  and therefore cannot close it. The misleading refusal text is folded in.
- **47 is MERGED (`a80fa87`) and fully closed**: demo gate Steps 1–5 all PASS,
  branch deleted locally and on `origin`, worktree removed, design/33 `:762` and
  `:912` corrected, `CLAUDE.md` updated, register row closed. **Camera ROI works
  again** after being refused on every map-carrying rig since `2599869`
  (2026-07-22).
- **Block 47's Step 3 is the measurement worth carrying forward.** The camera
  **accepted** a reposition entirely outside the current crop, so **MMCore's
  `setROI` takes full-frame coordinates** and the guard must not bound a
  requested ROI by the current one. The full-frame extent is still not portably
  knowable — `getImageWidth`/`getImageHeight` describe the image buffer — so
  microclaw guards integrality, nonnegative origin and positive size, and lets
  the adapter refuse its own sensor geometry.
- **The gate taught a rule now in `CLAUDE.md`: a new capability is not finished
  until it can appear in an exported script.** Restoring ROI produced sessions
  whose export died on line 17, because `set_roi`/`clear_roi` were undecorated —
...
- **With Track C parked on a human artifact, block 46 is assigned from the
  ...
  the operator ruling of 2026-08-11 rather than folded into a gated branch.
- **Block 46's register sweep was re-verified at `d938098` before assignment and
  still holds, with one correction**: there are **five** eventless exposure-write
  sites, not four — the old table folded `run_adaptive_survey` (`tools.py:5536`)
  under another row. Only the `set_exposure` row is measured; every other row is
  a hypothesis until a human sees the stale box.
- **Suite baseline at block 46's branch point, macOS: 1825 passed / 99 skipped /
  3 warnings, 1924 collected**, coordinator-measured at `d938098` — equal to
  block 45's close. On the branch after round 2: **1826 / 99 / 3 = 1925**, the
  one added test being the mid-acquisition GUI-failure case.
- **Block 46 is implemented, reviewed through one returned round, and pushed —
  it is awaiting a rig.** Runbook `design/46-block46-rig-gate.md` is on the
  branch, pinned `c68405a`. Round 1 returned four findings, **three in the
  runbook**, and two of those would have produced a false PASS: 7c routed to a
  different call site than the one it named, and 7d named a tool that does not
  exist. The ledger row carries all four and what round 2 did with them.
- **The coordinator's own assignment carried two wrong premises, both corrected
  by the implementer and verified before acceptance**: the eventless exposure
  writes are not per-tile, so the bridge-cost objection that shaped the block's
  policy question does not apply and "refresh every row" is right; and the fix
  needs eleven call sites, not six. **The block section's "six call sites" phrasing
...
| 9a | Features | **MERGED `4a750aa` 2026-08-26** — demo round 1, M5, demo round 2 all scored | `design26/generated-adapter-run-b` | `7d2bb9c` | `dcf3582` (Codex runner, worktree `../microclaw-9a`) + `e6c3db0` (review round 1, four blocking findings returned) + `c9c6b85` (round 2) + coordinator fixes `720eae5`, `d54b348`, `1ed6c27`; runbook `19378e8`/`884eda7`, re-pinned `7a5521a` to `1ed6c27`. **Round 1 returned four blocking findings and the two that mattered were architectural.** (1) `microclaw/` hard-coded one lab's project: `pool_probability_map` refused any labels but `{BG, apo_mito, healthy_mito}` and `analyzer_version` was the literal `"1.4.2"` — the same defect as tailoring to M5. Label roles became parameters and the version is read from the `.ilp`. (2) A **dead adapter class that could not work**: `IlastikPooledObservationAdapter` had no caller outside its own tests and its `analyze_frame` expected a probability map where a live hook receives a camera frame, so it would have raised on the first real frame; its test passed only because the test handed it the fixture. The runner flagged the tension itself and drew the wrong conclusion, and **the coordinator drew a wrong one too** — deleting the generated face is what left only a built-in, which the operator later challenged as contrary to design/26's premise (**resolved 2026-08-25: a committed hook library IS the destination, so the built-in stands; generation is proved later on a program we have not anticipated**). (3) The forbidden-parameter guard was disabled for **every** built-in, not just this one, though none of the adapter's parameters collide. (4) The absolute-path assertion tested `command[-1:]` — one element — while 'every path must be absolute' is the boundary's central rule. **Round 2 closed the last mock**: every committed test monkeypatched `h5py` out, so the four real key paths were correct but unguarded; a synthetic real-h5py pair now exercises them. **Verified by mutation**, not argument — renaming `exported_data` makes the new test fail and the mocked one still pass. **Three coordinator fixes the runner never saw.** `analyzer_version` was read from the `.ilp`, which records the version the project was *drawn* in, not the one that ran — true on macOS, potentially false on any machine with ilastik already installed, and the executable was not recorded at all; it is now `project_ilastik_version` beside `executable_path`, and **the rule is that a manifest must not state as fact a thing the run never checked**. Observations carried only their axis coordinate, so a ranked field could not be turned into a stage move; they now carry `stage_x_um`/`stage_y_um`. And round 1's own instruction (`read the version from the .ilp`) is what introduced the first of those — a fix that replaced a string that could lie with a field that lied differently. Suite 2100 + 99 at the branch point, 2119 + 99 at the pin, coordinator-re-run at every round rather than accepted — **the runner's round-1 report claimed two pre-existing failures that do not exist**, having run a system Python 3.10 with a broken `readline` and an ABI-broken h5py instead of the project's 3.11.15 environment; it retracted them | **Demo machine 2026-08-25 (`9a-gate-demo`): Setup, Steps 0, 1, 2, 3a PASS; Step 3b PASS on message and FAILED on cost; Step 4 clean for the adapter and dead for a pre-existing reason.** ilastik was already installed, `C:\Program Files\ilastik-1.4.2\ilastik.exe` reporting **1.4.2** — the same version the `.ilp` was drawn in, so no version caveat. **Step 0**: 2091 + 124 = 2215 collected, exactly macOS's 2116 + 99; the runbook predicted 2099 + 116 and **this machine skips 124**, which design/57's gate had already recorded — match the total, not the split. **Step 1 is the limb the trip was for and it passed on first contact**: from a sentence naming no tool the agent answered *"ilastik has a built-in offline adapter, so I don't need to write one"*, asked for the path instead of inventing it, asked to confirm the workflow, and volunteered unprompted that the survey had no signal so the numbers would describe the demo test pattern rather than structure — both the `4b` disambiguation and the never-treat-as-biological-validation instruction working. **Step 2**: 9 observations, all `unverified`, `whole_field`, version and executable recorded, a full 3×3 stage grid at ±512 µm, and **`launcher: None` — the Windows single-executable path no test and no run had ever taken**. 69.27 s for nine fields against 8.87 s on macOS, unexplained and worth a second measurement. **Step 3a**: sha mismatch refused in 0.14 s. **The gate's real finding is Step 3b: the right refusal 116 seconds late.** The configured-label check lived inside `pool_probability_map`, which runs only after the batch, so a mistyped label was refused *after* ilastik had scored every field — and the runbook had asserted the opposite all along. Fixed in `1ed6c27`, re-measured at **0.05 s** against the real project, with a test that asserts the subprocess never ran and **whose first version failed on the pre-fix tree for the wrong reason** (its fake wrote no output, so the old code died on a missing file before reaching the late refusal) — corrected, because a test that fails for the wrong reason is not evidence. **The gate's second finding is usability**: the session spent **four calls and two abandoned output directories** discovering that adapter arguments go in `parameters`, twice putting them in `model_project_config` — which sits beside it and looks exactly like where a project path belongs — while the generic hint told it to re-read a schema that described the *tool's* parameters, not the adapter's. Both fields are now described and the argument-error hint is specific. Compounding it, the output directory was created *before* the adapter was constructed, so each argument error left an empty directory that then blocked the name the caller retried with: **three runs left five directories**. Created after construction now. **Step 4**: six `run_analysis_on_saved_dataset` calls each emitted `# No hardware-routine effect`, no `import microclaw`, and the file parses — the adapter's emission is clean. The script still dies at line 25 on `NOT EMITTED: run_tile_acquisition — observation-only hooked timelapse has positions without recorded Z`, **which exists on `main`** and is the operator's ruling to leave for the open register. The runbook's Step 4 had expected zero, in a session its own Step 1 told the operator to create with a hooked tile acquisition. **Four runbook errors, all the coordinator's**, corrected in `1ed6c27`: the scoring block read `mito_coverage`/`apo_fraction` when round 2 had made the defaults generic and would have reported nine `None`s (the coordinator nearly filed that as a defect); `BG: confirmed` was expected though it depends on an optional `label_semantics`; Step 4's expectation as above; and Step 0's split. **Writing the runbook by running it had already caught three others** before the trip — a bash heredoc in a PowerShell block, a hard-coded path beside a variable the operator sets, and an `axis_selection` form that raises `TypeError: unhashable type` | **M5 with the real sample, 2026-08-25 (`9a-sarah-m5`) — the block's purpose demonstrated, and four defects found by scoring the artifacts rather than the outcome.** The whole chain ran: 3x3 and 5x5 surveys, ilastik scoring, `mark_position` from the recorded stage XY, a multiposition acquisition, and **nine apoptotic fields imaged and exported**. Every automatic decision held — `scale_matched` stride 1 from 0.1056 um against the project's 0.127, hash computed, executable discovered, nothing typed — and **the label refusal fired in 0.2 s** where the same mistake had cost 116 s that morning. 25 fields in 27.9 s, 49 in 35.9 s. **Defect 1 was the coordinator's, six hours old.** Scale matching read ilastik's `resolution: 1` — its placeholder when nobody sets a pixel size — as a real micron, chose stride 9, and decimated a 324x312 field to 36x35, below the project's own feature scales. ilastik produced nothing; the operator's **retrained M5 project failed three times**. Measured both ways on the rig's own `apo_1`: stride 9 exits 1 with `FeatureSelectionConstraintError` and no output, stride 1 exits 0. **1.0 is now treated as unset, and an unknown scale keeps every pixel** rather than falling back to a fixed target — guessing a stride for a trained classifier fails silently, and the fallback that existed *was* the guess. **Defect 2 hid defect 1**: the failure surfaced as `did not create expected output` while ilastik's own reason sat in captured output we discarded, so the session retried three times diagnosing flaky I/O. Both failure paths now carry its last lines. **Defect 3 was never hit and is the dangerous one.** The retrained project names three labels and trained two (`known_labels: [1, 2]`); ilastik exports an **all-zero channel** for a class nobody drew, so `apo/(apo+healthy)` would have returned **1.000 for every field** — a confident '100% apoptotic' that is an artifact, with nothing in the artifact looking wrong. A ratio against an untrained class now refuses in 0.1 s naming the trained set. **This is the `known_labels` check deferred that morning, and it mattered for channel VALIDITY, not the channel ORDERING the coordinator predicted** — the existing count check already covered ordering. **Defect 4 sits on the retraining path**: `export_dataset_as_tiff` did not create its output parent, so nine exports failed and the generic hint asserted *microclaw creates parents itself* and named three causes that were all wrong; the agent recovered by writing a README into the folder. Exporting frames is how images reach a classifier for training. **Two operator hypotheses were tested and refuted from the artifacts.** The failure was *not* an NDTiff problem: the exported TIFFs Sarah trained on are **bit-identical** to the NDTiff frames the adapter scores (same shape, same dtype, `array_equal`), and the same datasets worked with the original project throughout. And the coordinator's own suspicion that the 0.01 `coverage_floor` was mis-set is **refuted**: 60-88% of fields came back `unresolved` because the M2 classifier does not transfer — on a field the operator called apoptotic it assigns **twice as much healthy as apoptotic** probability (BG 0.969 / apo 0.010 / healthy 0.021, coverage 0.031) against the retrained project's coverage 0.095. The gate was honestly reporting a classifier that does not transfer, which is the domain shift **measured rather than predicted**, and it vindicates the decision to retrain. All four fixes replayed against the rig's own artifacts: the retrained project refuses in 0.1 s on the untrained class, completes in 9.5 s against a trained denominator, and **all nine NDTiff datasets score cleanly** (coverage 0.085-0.159, 82 s). Suite 2135 + 99. **Demo round 2 assigned** (`32f97cc`) covering only the agent-facing surface the fixes have not exercised | **Demo round 2 2026-08-26 (`9a-gate-demo-round2`) — GATE PASS, all four steps.** R1 `2110 + 124 = 2234`, equal to macOS's 2135 + 99. **R2 PASS cleanly**: nine exports into a folder that did not exist, **first attempt, zero errors in the whole session**, and no folder created by other means — the workaround that would have masked the fix did not occur. **R3 PASS**: all nine `unknown_scale_no_decimation`, stride 1, `native_pixel_size_um: 1.0`, `project_training_resolution_um: null`; `scale_matched` never appeared, which is what the M5 defect would have looked like. **R4 PASS, and it did more than the criterion asked.** Refused in **0.08 s** with the exact message. Before running the adapter the agent had already seen `healthy_mito` read 0.0 on every tile and explained it as *"consistent with this being the DemoCamera synthetic pattern"* — a plausible, biological, **wrong** account of a missing annotation. After the refusal it corrected itself in its own words (*"exists as a label name in the project but was never actually trained ... that's why it scored 0.0 everywhere in the earlier runs"*), declined to retry the other eight because *"the refusal is a property of the project, not the data"*, and offered retraining or `apo_mito ÷ BG` rather than quietly switching denominator. **So the refusal overturned a causal story the agent had already committed to**, which is a stronger result than preventing the 1.000 artifact it was designed for. Caveat for honesty: the operator's prompt said *"I expect the tool to fail quickly"*, so the failure was primed; the handling is what the step tests. **Two runbook defects of the coordinator's, both harmless here.** Setup asserts the *original* project's hash while round 2 instructs copying the M5 one, so the check tripped and the runbook said to stop; the operator correctly proceeded. And the M5 hash the runbook quoted (`55c2242e`, 378873 B) is a **different file** from the one used (`764556ce`, 376425 B) — the project was re-saved that evening. **Asserting a hash for an artifact the operator supplies from their own machine is the wrong design**; it should be reported, not asserted. The file used was verified to carry both required properties, so the round stands. **One finding, recorded in the register and deliberately not opened**: the survey was written as nine separate datasets by `run_tile_acquisition(protocol="timelapse")`, so the batched boundary ran nine times — ~17 minutes against ~90 s for one batch, because **ilastik start-up is 60–90 s on Windows against 5–7 s on macOS** | | |
| 9b | Features | — | **none — closed unstarted** | | | **CLOSED as not needed, 2026-08-26, by operator decision.** Its purpose was struck first: the verdict on whether ilastik beats the classical floor is a research question about ilastik, not a product question about Microclaw (operator ruling 2026-08-25; design/26 F5 corrected in place). What remained was one shipped default, `coverage_floor = 0.01`, and it did not justify a block — it is already a parameter, every observation records `coverage` beside `coverage_floor` so a user can see and override it, and the same reasoning that struck the verdict says a validity threshold is the microscopist's judgment about their own sample. **Measured spread across 9a shows why no universal value is obvious**: 0.0005–0.008 with a non-transferring classifier, 0.085–0.159 with a working one, 0.106 on synthetic frames — the variance is mostly *which classifier*, not *which field*. **One question is recorded rather than scheduled** in §9b: whether an empty field and a populated one separate on coverage at all — answerable offline from the 108 archived fields, worth doing only if the gate is ever suspected of hiding good fields, which the evidence so far does not suggest | n/a | n/a | n/a |
| 10 | Features | 9a (9b closed); optional | `design26/few-shot-run-c` | | | required or marked skipped | | |
...
| 66 | Register promotion | design/64 and 64c merged | `design66/stage-move-response` | `fa9b3cf` | `ff6723c` + runbook `8156eb4` (Codex runner, one interrupted turn resumed); review round 1 = `78728f2` (coordinator, gate) + `996ae3c` (Claude runner, product, merged `2a85841`) + `47331a4`; gate findings `8eae845`, `9da7d61`, `4527251`, `97af8f9` | **M2 2026-08-30 PASS on the success limb; demo 2026-08-30 PASS; the genuine non-response control has NO rig evidence** and was closed off-rig by operator decision, on a defect the rig itself found. **M2 success**: `TIRF Stage` −0.1 → 199.9, `measured_um` 199.2, `arrival_residual_um` **0.7**, `tolerance_um` **20.0** recomputed from the record's own start and target, `band_source: relative`, **0.453 s**. The residual is above 0.5, so this exact move is one the old constant refused; the same stage's 0.7 µm residual on 2026-08-29 consumed ~10.1 s and raised. The relative rule — not the floor — was the thing exercised. **Demo**: `Aux Z` 0.0 → 60, exact arrival, `tolerance_um` 6.0, `band_source: relative`, 0.125 s, and the **selected export** M2 never managed: selection header, two `# SKIPPED` ids, one emitted move. Its emitted literal is `settle_stage_move(core, 'Aux Z', 60, 0.0, 'relative', None)` — an **integer** target, the exact case round 1's structural matcher replaced a substring match to catch, earning itself on real evidence rather than in a fixture. **The control limb found a product defect, which is what a control is for.** Disconnecting the controller produced neither predicted refusal: a link that is down fails *every* bridge call, so the pre-dispatch read raised before `set_position` and an untyped `java.lang.Exception` escaped carrying no `start_um`, no `band_source` and no typed class. This block had widened that exposure — five sites gained a pre-dispatch read that had none. All six now read through `read_stage_start_position`, inlined by `_stage_move_contract_source` because `sweep_autofocus`/`_restore` are emitted verbatim; a relative move with no readable start reports `requested_um: None` rather than a fabricated coordinate, and `move_named_stage` reads **once** again, its two reads having been separated only by an in-process guard check. Eight parameterized cases watched failing on the pre-fix product. **M2's own control record still scores NOT EXERCISED** — that session ran the unfixed code, and the scorer does not retroactively pass it. **Two more product defects came from the gate sessions, not the suite**: `export_session_script` invited an agent to pass a tool *name* as an id (it did, was refused, and recovered by exporting the **whole session** — the dangerous direction, which passed only because that session held nothing else emittable), and it reported plain success while emitting nothing when every selected id was real but non-emitting. Both now say what they are. **Round 1's nine findings were five gate and four product**, and the gate half is the story: the scorer could not parse a single real Microclaw history, the control limb could never have been scored at all, and the bridge-shaped self-test hid both by writing the history the scorer expected — 60b's glob, reproduced. Suite **2601 passed / 99 skipped / 3 warnings**, coordinator-run; self-test 21 limbs, 7 of which fail independently on the pre-fix tree | `2cbf247` | design/66 §"Rig gate" and §"`start_um` and band policy" reconciled to what was measured; design/55 register row 1 **closed**; the cross-plane non-response row stays **open** with its owner |
| 64d | Register promotion | design/64 merged; 64c merged; **design/66 merged `2cbf247`** — design/67 deferred this row explicitly to "that block, after design/66 lands" | `design68/xy-arrival-contract` | `2cbf247` | `b14cd0a` (preserved from a killed runner, **committed unreviewed**) + `87af292` + `7cc33cc` (Codex runner, one interrupted turn) + `33828a6` (coordinator review round 1) + gate `3234c68` | **M2 2026-08-31, two runs, all six limbs exercised.** Run 1 `gate68-m2` limbs 0/A/B/D/E **PASS**: an ordinary 20 µm move **settled in 0.984 s** with `measured_um` `[55.7, 225.6]` for `requested_um` `[55.5, 225.7]` — measured is not requested, which is the block's whole claim; a 200 µm X move earned a **20.0 µm relative** band while the held Y kept the **2.0 µm floor**; and `center_feature` converged 85.7 → 3.04 px through a **1.5875 µm sub-band correction** that reported `arrival_unverifiable` and was not refused (12.5 px × M2's 0.127 µm/px = 1.5875 µm — the numbers cross-check). Run 2 `gate68-m2-r2` limb C **PASS** with the Core XY device disconnected: typed `XYStageMoveError` naming `['x','y']`, `elapsed_s: 0.0` from `read_xy_start_position` **before any write**, `requested_um: null` rather than a fabricated coordinate. **Block 66's untyped-escape defect does not reproduce for XY.** Round 1 of limb C was a **gate** failure scored NOT EXERCISED, not a product one — see below | `19d02c0` | **Step-2 deviation, recorded not hidden:** the first implementation turn was a Claude runner in its own worktree that the account spend limit killed mid-turn with 648 lines uncommitted and no report. Per `CLAUDE.md` the coordinator verified the tree, preserved it as `b14cd0a` with a message saying plainly it is unreviewed, and handed it to the Codex turn with that in writing. **Sixth killed turn** the workflow has seen; its revision turn was also killed, after committing `7cc33cc`. **Review round 1 found the block's central test could not fail:** the delayed-arrival limb had its late axis arrive on exactly the third poll, which is `STAGE_MOVE_REQUIRED_SAMPLES`, so a loop that ignored the per-axis band entirely and merely collected three readings returned the same value. Established by mutation (`if True:` in place of the band check left both parameterizations green), fixed in `33828a6`, and the same mutation now returns 71.0 where 99.0 is asserted. **The gate self-test found one gate defect before the rig did and MISSED another:** limb C commanded a move along X only, caught off-rig; but its `entry = read_xy(...)` sat outside its own `try`, so on M2 the disconnected link raised there and the limb died in its instrumentation without ever calling `move_stage_xy`, reporting FAIL where the truth was NOT EXERCISED. The self-test hid it because `--untyped-failure` asserted only *that limb C reported FAIL* — which it did, for the wrong reason. **A criterion satisfied by the wrong mechanism, in the gate written to enforce that rule.** The mode now requires the limb to have reached the product. Two smaller gate defects from the same run: a raw Java traceback where NOT EXERCISED was correct, and the control phase overwriting run 1's results file. Suite **2617 passed / 99 skipped / 3 warnings in 176 s**, coordinator-run; branch point was 2601/99/3 in 135 s, and the +35 s is the arrival loop, measured on the product-only commit before any new test existed. |
| 65a | Adaptive STORM | design/65 written | `design65/smlm-skill-accuracy` | `c0629c1` | | **none — no rig surface.** The change is to a shipped documentation file; what it must not do is *promise* a capability, which is checkable by reading. If a rig limb is wanted it is a driven session that asks for adaptive density control and is told the truth | | |
| 65b | Adaptive STORM | 65a merged | | | | **M2 required** — design/65 §"Rig gate": three operator-judged limbs (successor dispatch + early stop with no hardware capability; an image-derived write to a non-dosing property; a short authorized `Duration0` run on a frame-index predicate), everything that only computes in one script run against `design/55-gate-probe-selftest.py`'s bridge-shaped fake first | | |
...
# Block 46 — [~] the GUI-refresh sweep — **DROPPED unmerged 2026-08-12**

Branch: `design46/gui-refresh-sweep` — **deleted locally and on `origin`, never
merged.** No code from this block is on `main`.

> **Dropped by operator ruling after its own gate refuted it.** The block was
> implemented and reviewed through one returned round, and the code was sound;
> the *premise* was wrong. Micro-Manager publishes `PropertiesChangedEvent` and
> repaints itself — measured on the demo machine against `main`, which contains
> no repaint in `set_exposure` at all and updated the box anyway. The M5 sighting
> was a single unreproduced session, and block 46's write-time repaints could not
> have fixed it regardless, because that session made no exposure write. The
> existing `refresh_gui()` calls on `main` are block 43b's **EMU** fix, are
> measured, work, and are untouched by this drop.
>
> Full reasoning, and the investigation to run **if it ever recurs**, are in the
> carried-forward register entry "The GUI stops tracking after an *exposure*
> write". Read that before re-proposing anything here.
>
> The section below is kept as the round record. **Do not act on it.**

Scheduled 2026-08-12 by operator decision, with Track C parked on an `.ilp` that
does not exist yet. It is design/43 F4's class, carried out of 43i deliberately
rather than folded into a gated branch (operator ruling, 2026-08-11).

**The one measured fact.** M5, 2026-08-11, TIRF session on 43i's branch:
microclaw read exposure 20 ms while Micro-Manager's Exposure [ms] box showed 50,
and Tools → Refresh GUI revealed the 20. **The origin of that specific 20 ms is
undetermined and must not be guessed** — it is absent from that session's history
and from 43i round 2's, so it predates both, which is itself consistent with a
stale GUI persisting across sessions until something refreshes it. Do not build
the block on a theory of where it came from; build it on the staleness.

**The register's sweep was re-verified at `d938098`, 2026-08-12, and still
holds** — with one correction. Four tools refresh (`_set_channel_for_composite`
`tools.py:2081`, `set_device_property` `:2169`, `set_focus_lock` `:6893`,
`set_emu_laser_power_percentage` `:7049`), plus two sites inside
`execute_channel_plan` (`authorization.py:1731`, `:1752`). Nothing else does.
Every writer below still lacks it:

| writer | what goes stale | evidence |
| --- | --- | --- |
| `set_exposure` (`tools.py:1727`) | Exposure [ms] | **measured** |
| `set_roi` (`:1785`), `clear_roi` (`:1804`) | ROI | inspection |
| `move_stage_xy` (`:1832`), `move_stage_z` (`:1883`), `move_named_stage` (`:1966`) | stage position display | inspection |
| eventless exposure writes: `run_zstack` (`:2583`), `run_timelapse` (`:2733`), `_acquire_positions_with_hook` (`:4883`), `_acquire_survey_with_detector` (`:5227`), `run_adaptive_survey` (`:5536`) | Exposure [ms], mid-run | inspection |

**The correction: that is five eventless exposure writes, not four.** The
register's table folded `run_adaptive_survey` under another row; it is its own
call site and an implementer working from the old table would miss it.

**The block's actual question is which of these to repaint, not whether it can
be.** `refresh_gui` is best-effort, never raises, and repaints from a cache that
is already current after a core write (`controller.py:321`), so the fix is cheap
wherever it is wanted. Do not turn this into a framework.

- [ ] **Decide the policy before writing the calls, and write the reasoning
      down.** A repaint after every stage step in a survey is a per-tile bridge
      round trip on a serialized bridge — the mid-run rows are not obviously
      wanted, and "refresh everything" is a defensible answer only if it is
      argued rather than assumed. Recommend a disposition per row.
- [ ] Fold into the existing `ctrl.refresh_gui()` call. **Do not add a decorator,
      a wrapper, a registry of writers, or a guard pass** — six call sites in the
      files that already write are the whole change.
- [ ] Each inspection row needs its **own** one-glance rig check before it is
      called a defect. An unrefreshed writer is a hypothesis until a human sees
      the stale box.
- [ ] Do not change what any writer reports or returns. This block repaints a
      display; it must not alter a single tool result, and the suite must show
      that.
- [ ] Confirm the no-raise contract still holds where the new calls land: a GUI
      failure must never turn a successful hardware write into a failed tool
      call, including mid-acquisition.
- [ ] **Check the export path.** If a repaint lands inside an emitted tool's
      recorded effects, the standalone script must not acquire a `microclaw`
      import or a `refresh_gui` it has no definition for.

Rig gate: the measured row plus one glance per inspection row. **The demo machine
can carry most of it** — a stale Exposure box is a stale Exposure box — but say
which rig each row actually ran on rather than claiming demo sufficiency, which
is the citation error block 45's gate produced.

Post-merge design gate:

- [ ] Correct design/43 F4's account to what was measured, and record the
      per-row disposition — including any row deliberately left unrefreshed,
      **with its reason**, not as an empty checkbox.
- [ ] Rewrite this file's open-register entry to the block's outcome and
      re-count the register.

---

# Block 47 — [x] give camera ROI a typed capability — **MERGED 2026-08-12**

Branch: `design47/camera-roi-capability` — merged `a80fa87`, deleted locally and on `origin`

> **Demo gate: all five steps PASS.** Step 3's opposite-corner request was
> **ACCEPTED** by the camera — from a crop at `(0,0,128,128)`,
> `set_roi(384,384,128,128)` succeeded and `get_roi` confirmed it — which
> measured that MMCore's `setROI` takes full-frame coordinates and that review
> round 1's containment rule would have refused a valid operation. Step 4's
> `width=0` was refused by our own guard before hardware, ROI unchanged.
> **The gate also found that ROI could not reach an exported script**: both
> tools were undecorated, so the export died on line 17. Emitters folded in
> before merge (`dcbc8cb`); undecorated tools 14 → 12.

Scheduled 2026-08-12 by operator decision, from the open register row "Camera ROI
has no typed capability". It is promoted out of the register because **it is a
working feature that silently stopped, found by an operator on their own machine
while running another block's gate** — the same pedigree as block 46.

**ROI worked for a month and was not removed on purpose.** `get_roi`, `set_roi`
and `clear_roi` landed 2026-06-20 (`55e9eb8`). The Phase-1 authorization map
landed 2026-07-22 (`2599869`) and emitted `camera-roi` as `excluded` because no
typed ROI capability existed to classify it. design/33 recorded the intent
honestly — "a separate geometry capability, already tracked" (`:762`), "tracked
for a later phase" (`:912`) — and that phase was never scheduled. This block is
that phase.

**What is actually broken.** `authorization.py:1364` appends
`AuthorizationEntry(path="camera-roi", classification="excluded", …)`
**unconditionally**. It is hard-coded, not derived from `safety_config.yaml`.
`set_roi`/`clear_roi` call `authorize_path(ctrl, "camera-roi")`
(`tools.py:1795`, `:1808`), which refuses on either an excluded entry **or no
entry at all** (`authorization.py:1775`). Reads are unaffected — `get_roi` never
calls `authorize_path`. The exclusion bites only where a map is attached:
`authorize_path` returns early when `ctrl.authorization_map` is `None`, so ROI
still works on an install with no map.

**Confirmed on the demo machine 2026-08-12**, during block 46's Part A, with the
refusal text quoted in that gate's evidence.

- [ ] **Replace the hard-coded `excluded` entry with a real typed capability.**
      Fold into the existing `AuthorizationEntry` emission beside
      `dedicated-exposure` (`authorization.py:874`) — **do not add a registry, a
      validator layer, or a second authorization concept.**
- [ ] **Decide where ROI bounds come from, and justify it.** Recommendation, not
      a decision already taken: derive them from the camera's own full-frame
      dimensions rather than adding a config surface. The sensor knows its size,
      and making an operator hand-declare pixel bounds before ROI works again
      would repeat the authoring gap this whole checklist exists to close. If the
      block concludes a declaration is genuinely needed, say why in the design
      gate.
...
- [ ] **ROI is geometry, not dose.** It does not change illumination, and frames
      ...
      rate changing achievable frame rate inside a reservation), say so
      explicitly rather than assuming either way.
- [ ] **Fix the refusal text, which actively misdirects.** `authorization.py:1778`
      tells the operator *"the rig's safety config would have to change first"* —
      false for a hard-coded exclusion, and it sent the demo operator hunting for
      a setting that does not exist. An excluded path with no config remedy must
      say so. Folded in here rather than opened as its own register row: same
      function, same call, and splitting them would be artificial.
- [ ] Confirm `get_roi` stays unauthorized and unaffected; a read must not start
      requiring a capability.
- [ ] Check the export path: an emitted standalone script must not acquire a
      `microclaw` import or an authorization call it has no definition for.

Rig gate: **the demo machine carries this whole block** — ROI is a camera
geometry operation and the demo camera has a sensor. Note that block 46's A2 is
the same code path, so 46's gate cannot close it and this block's gate is where
ROI is proven. Say which machine ran it rather than claiming demo sufficiency.

Post-merge design gate:

- [ ] **Correct, do not annotate**, design/33 `:762` and `:912` — "tracked for a
      later phase" is no longer true once this merges. Record what the typed
      capability actually is.
- [ ] Tick the carried-forward register row "Camera ROI has no typed capability"
      and record the refusal-text fix against it, so block 12's closeout does not
      re-litigate a closed row.
- [ ] State plainly that ROI was unavailable on map-carrying rigs between
      `2599869` (2026-07-22) and this merge, so nobody reading the history
      concludes it was a deliberate capability decision.
...
**Intake re-held 2026-08-24, when the `.ilp` arrived. Three of the four items are
answered and one recorded answer was wrong.**

| intake item | answer |
|---|---|
| what the target means | **apoptotic vs healthy mitochondria, selecting whole fields** (operator, 2026-08-24). **This replaces the 2026-08-12 answer of "cell / puncta counting", which the `.ilp` refuted** — its labels are `BG`, `apo_mito`, `healthy_mito` |
...
| a trained `.ilp` | **exists, 2026-08-24** — `sarah-ilastik-training-file/260824_Mito-classify.ilp`, sha256 `a9a63447…c639ce` |
| a known input and its known result | **still owed.** Held-out fields marked mostly-apo / mostly-healthy / unusable; the two training lanes cannot serve, the classifier saw them |

**The unit is a per-field pooled score, because that is what the operator selects
on.** `apo_fraction = Σp(apo) / (Σp(apo) + Σp(healthy))` is the ranking axis, and
`mito_coverage = 1 − mean p(BG)` is a **validity gate, not an extra field** — a
field with almost no mitochondria has an unstable ratio and would rank as
confidently apoptotic on a handful of pixels. Same shape as the SNR-gated focus
metric. Per F3 the "mostly" threshold is the one part that does not compile, so
the hook logs the ratio and the threshold comes from the fixture.

**What the `.ilp` measures, coordinator 2026-08-24** (ilastik 1.4.2, the version
F5 measured and the one installed): Pixel Classification, genuinely trained — 100
trees over 8 forests, 22 features, 1887 samples. Headless scored one 220×512 frame
in **8.18 s**; `exported_data` is `(220, 512, 3)` float32, axistags `y,x,c`,
channels in `LabelNames` order and summing to 1.0.

**Three labels, so this lands in the case F5 flagged and never tested.** With two
labels the channels are complements and a fitted probe learns its own polarity;
here they are a 3-way softmax, so the channel→class mapping is **load-bearing**
and the adapter must read `LabelNames` rather than assume an order.

**A scientific limit is visible in the labels, and it belongs in the verdict.**
Lane 0 (`IF27_spl09_pos01_apo`) carries `BG` + `apo_mito` only; lane 1
(`Pos0_IF26_spl01_healthy`) carries `BG` + `healthy_mito` only — different samples,
different sessions (IF26 260612, IF27 260805). **No single training image contains
both mito classes**, so the classifier may be separating sessions — background,
illumination, focus — rather than morphology, and nothing inside the `.ilp` can
tell those apart. If any one session holds both an apo and a healthy field, that
pair is the fixture worth more than any other.

Training data are Micro-Manager OME-TIFF (`MMStack`), 488 WF, 0.127 µm/px, from
**M2** — so M2 is the gate rig for the eventual survey.
...
> **Amended 2026-08-24 — the prediction flips, the recommendation does not.** That
> "likely negative" rested on puncta separating *photometrically*. Fragmented vs
> networked mitochondria is a shape concept — F9's oriented/configural rung, which
> F5 named as the only honest place to ask whether ilastik buys anything — so this
> target is a real contest rather than a rerun. **Measuring the floor first is
> still right**, because the verdict *is* that comparison and it cannot be written
> without one. **Operator chose floor-first, 2026-08-24.** Note the floor is not on
> `main`: `ClassicalDescriptor`, `FewShotProbe` and `detectors.py` are design/26
> stubs, and the only working implementation is `design/26-roi-detection-spike.py`
> — so scoring it is a spike run folded into the design doc, not a product path,
> and it needs no branch. Its section E already builds the oriented/configural
> case, which is the precedent for this target.
...
## 9b. Design/26 Run B — the coverage floor — **CLOSED, not needed (2026-08-26)**

**Closed by operator decision, unstarted and unbranched.** Nothing here is
deferred or owed; this section records why, so a later reader does not mistake a
decision for an oversight.

**Its original purpose was struck first.** 9b existed to settle whether ilastik
beats the classical floor. That is a research question about ilastik, not a
product question about Microclaw — the microscopist owns the session and is
entitled to use the tool they judge right for their sample (operator ruling
2026-08-25; design/26 F5 corrected in place). The floor spike, the adjudicated
fixture, the M2 survey and the verdict all went with it.

**What remained did not justify a block.** One shipped default — `coverage_floor
= 0.01`, below which the adapter reports `unresolved` rather than an unstable
ratio — and three reasons not to spend an afternoon on it:

* **It is already a parameter**, and every observation records both `coverage` and
  `coverage_floor`, so a user who gets `unresolved` can see exactly why and
  override it.
* **The same ruling that struck the verdict argues against tuning it centrally.**
  A validity threshold is a scientific judgment about the user's own sample, and
  design/26 F3 already holds that the threshold is the one thing in this pipeline
  that does not compile.
* **There may be no good universal value.** `coverage = 1 − mean p(BG)` is
  dominated by how much of a field is genuinely background, which varies with cell
  density, magnification and classifier. Measured across block 9a: **0.0005–0.008**
  with a classifier that does not transfer, **0.085–0.159** with one that does,
  **0.106** on synthetic demo frames — the spread is mostly *which classifier*,
  not *which field*.

**The one question that would have had value is recorded here rather than
scheduled**: nobody has checked whether an empty field and a field full of cells
separate on coverage at all. If they do not, the gate is decorative and the
mechanism — not the constant — is what should change. It is answerable offline
from the 108 fields block 9a left in the archive (`9a-sarah-m5`), needs no rig,
and is worth an afternoon **only if the gate is ever suspected of hiding good
fields**. On the evidence so far it is not: it correctly flagged a classifier that
was not working, and passed everything scored by one that was.

**Block 10's dependency on "9" is satisfied by 9a alone**, and block 11's
"accepted Run B fixtures" are 9a's committed fixtures.
...
| Camera ROI has no typed capability | design/33 `:708`, `:831` | **CLOSED — Block 47, merged `a80fa87` 2026-08-12.** Typed geometry capability shipped and demo-gated; the misleading "the safety config would have to change first" refusal fixed in the same block; `set_roi`/`clear_roi` now emit, so ROI reaches a standalone script. ROI was refused on every map-carrying rig from `2599869` (2026-07-22) until this merge — a gap, not a capability decision |
...
| XY stage motion has no arrival contract | design/63 block 63a, 2026-08-30 | **CLOSED 2026-08-31 by block 64d** (design/68), M2-gated over two runs — design/56 gave the single-axis tools a measured read-back; `move_stage_xy` still dispatches, calls `wait_for_device` and *reports* the residual, so `center_feature`'s closed loop can snap before the stage arrives. 63a's first round widened `settle_stage_move` to fix this in the *emitter*, which was rejected: an exported step must not be stricter than the tool it reproduces, and the fix belongs in the tool. **design/64 (merged 2026-08-30) did not close this and made it more exposed** — `center_feature` now snaps immediately after each correction, and its loop re-resolves the affine per iteration. Needs a rig gate for stage motion. |
...
| `tol_px` is not checked against the stage's step size | design/64, M2 2026-08-30 | **CLOSED 2026-08-30 by block 64c** (design/67, merged 4394338) — the loop now reports `residuals_px`, `residual_offset_um` and `smallest_correction_um`, and classifies a non-convergence as rising / plateaued / still-falling instead of always blaming the calibration. No step size is learned, configured or enforced — M2 quantizes ~0.8 µm (design/29), which at 0.127 µm/px is a **6.3 px floor against `center_feature`'s 5.0 px default**, so convergence there is partly luck: limb F needed one correction (72.3 → 2.5 px) and limb F2 needed three (47.0 → 3.9). The non-convergence `hint` blames a stale calibration and never mentions the stage. Smallest of the three, and the M2 data to size it already exists in `gate64-m2`/`gate64b-m2`. |
| The knowledge base has a lost-update race between concurrent launches | design/64, 2026-08-30 | **CLOSED 2026-08-30, no rig needed** — `save_entry` and `delete_entry` now share one `_update_knowledge`, which loads, mutates and rewrites **inside** an exclusive OS lock on a sidecar `knowledge.yaml.lock`. Sidecar because `_write_knowledge` replaces `knowledge.yaml` itself, so a lock on that inode would guard nothing; an OS lock rather than a marker file because the kernel drops it when a killed launch exits. Reproduced first: `design/64-kb-lost-update-probe.py`, two processes writing 40 disjoint keys each, **lost 40 of 80** on the unfixed tree and 0 of 80 after. The wait is bounded at 10 s and raises — Windows' `LK_LOCK` already gave up after ten seconds while POSIX `flock` blocked for ever, and a behaviour that exists only on the rig is one nobody can test. **Readers stay lock-free** and keep the atomic snapshot `_write_knowledge` gives them. |
...
- **The GUI stops tracking after an *exposure* write, and six other write paths
  ...
  **Origin of the specific 20 ms is undetermined and should not be guessed.** It
  ...
  > **CLOSED as "not a standing defect" — block 46 assigned, implemented, gated
  > and DROPPED unmerged, 2026-08-12, operator ruling.** Four things are now
  > known and the row should not be re-opened without the first of them.
  >
  > **1. Micro-Manager is supposed to repaint by itself.** It publishes
  > `PropertiesChangedEvent`
  > (`https://micro-manager.org/apidoc/mmstudio/2.0/index.html?org/micromanager/events/PropertiesChangedEvent.html`)
  > and the GUI listens. **Measured on the demo machine on `main`** — whose
  > `set_exposure` is `guard → core.set_exposure → return`, containing no repaint
  > at all — the operator set the box to 50, ran one `set_exposure(20)`, and the
  > box updated to 20 unaided. The premise that microclaw must repaint after
  > every write is wrong as a general rule.
  >
  > **2. The existing `refresh_gui()` calls are an EMU fix and must stay.** Block
  > 43b's six sites (`tools.py:2081`, `:2169`, `:6893`, `:7049`, and
  > `authorization.py:1731`, `:1752`) exist because **EMU** sometimes will not
  > repaint without an explicit call. That fix was measured, it works, and
  > `set_emu_laser_power_percentage` at `:7049` is the laser-property case
  > specifically. **Do not "clean up" or generalise these away.** They are not
  > the same claim as this row.
  >
  > **3. The M5 sighting was one session and has not recurred.** Most likely a
  > property refresh stuck once, in the main GUI's exposure box rather than in
  > EMU. One unreproduced event is not a standing defect.
  >
  > **4. Block 46's design could not have fixed it anyway.** All eleven proposed
  > calls hung a repaint on a *write*, and the M5 session made no exposure write
  > — no `set_exposure`, no `exposure_ms`, four `run_timelapse` calls passing
  > none. Not one would have fired. This was derivable at assignment from the
  > paragraph directly above and the coordinator did not derive it.
  >
  > **If it recurs, this is the investigation** — do not re-assign the sweep.
  > On M5, in a **fresh session**, change each property **individually** and
  > record what the GUI did after each one: exposure, ROI, XY position, Z
  > position, each named stage, and an EMU laser property as the known-good
  > control. Write a description of what was seen per property, not a pass/fail
  > tick. The question is *which* property stopped publishing its event and
  > whether it is reproducible at all — not whether microclaw should repaint
  > everything.
