...
def test_measured_gap_distribution_is_bounded_in_progress_and_caller_state(monkeypatch):
    ...
    from tests.test_timing_attribution import AXIS
    assert tools._GAP_HISTOGRAM_UPPER_S == AXIS
    assert all(row["count"] > 0 for row in summary["nonzero_histogram"])
    assert sum(row["count"] for row in summary["nonzero_histogram"]) == 2
