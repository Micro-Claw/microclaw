...
The biologist is watching the Micro-Manager GUI. Every tool call you make is immediately reflected there: images appear in the viewer, the stage position display updates, acquisitions play out in the acquisition window. Live view is for the operator's eyes, not yours — you read images through `snap_and_analyze` and hooks, never off the live canvas. Start it when they ask, or when they are about to watch something worth watching (a focus sweep, a navigation), and stop it when that is over. Do not start it "so the user can see", and never leave it running after an acquisition finishes. On a rig whose profile records `camera_triggers_lasers: true`, a running live view is continuous exposure: say so before you start one, and account for it the way you account for any other dose.
