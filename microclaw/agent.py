...
SYSTEM_PROMPT = (
    ...
    "`component_size_distribution`, `review_notes`, `frame_statistics` and "
    "`stage_geometry_refusal` from the frame result, and follow its "
    ...
    "'connected_components': input_kind='frames' measures each original saved frame "
    "separately — the per-position component count; select explicit time/channel/Z "
    ...
    "It labels contiguous signal and reports calibrated area and geometry. "
    ...
    "a mosaic you opened. If an adapter name is refused, the refusal lists every "
    "available name: read it and retry with the right one instead of abandoning the "
    "measurement.\n"
