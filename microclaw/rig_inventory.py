...
def _property_record(core: Any, device: str, prop: str, failures: list[dict]) -> dict:
    ...
    local: list[dict] = []
    value = _query(local, scope, "current_value", lambda: str(core.get_property(device, prop)))
    if _SECRET_NAME.search(prop):
        value = _REDACTED if value is not None else None
    allowed = _query(local, scope, "allowed_values", lambda: sorted(_strings(core.get_allowed_property_values(device, prop))))
    if _SECRET_NAME.search(prop) and allowed is not None:
        allowed = [_REDACTED] if allowed else []
    record = {
        "name": prop,
        "current_value": value,
        "read_only": _query(local, scope, "read_only", lambda: bool(core.is_property_read_only(device, prop))),
        "pre_init": _query(local, scope, "pre_init", lambda: bool(core.is_property_pre_init(device, prop))),
        "allowed_values": allowed,
        "has_limits": _query(local, scope, "has_limits", lambda: bool(core.has_property_limits(device, prop))),
        "reported_type": _query(local, scope, "reported_type", lambda: str(core.get_property_type(device, prop))),
    }
    if record["has_limits"]:
        record["technical_range"] = {
            "lower": _query(local, scope, "lower_limit", lambda: float(core.get_property_lower_limit(device, prop))),
            "upper": _query(local, scope, "upper_limit", lambda: float(core.get_property_upper_limit(device, prop))),
...
def _preset(core: Any, group: str, name: str, failures: list[dict]) -> dict:
    ...
    if config is not None:
        ...
        for index, setting in enumerate(settings):
            effects.append({
                "device": _setting_value(setting, ("device", "device_label", "get_device_label", "getDeviceLabel")),
                "property": _setting_value(setting, ("property", "property_name", "get_property_name", "getPropertyName")),
                "value": _REDACTED if _SECRET_NAME.search(str(_setting_value(setting, ("property", "property_name", "get_property_name", "getPropertyName")) or "")) else _setting_value(setting, ("value", "property_value", "get_property_value", "getPropertyValue")),
                "setting_index": index,
            })
...
def enumerate_rig(core: Any, *, mm_config: str | Path | None = None) -> dict:
    ...
    for field, method in (
        ...
        assignments[field] = _query(failures, "core_assignments", field, lambda m=method: str(getattr(core, m)() or ""))

    devices = []
    labels = _query(failures, "core", "loaded_devices", lambda: sorted(_strings(core.get_loaded_devices())), [])
    for label in labels:
        scope = f"device:{label}"
        kind = _query(failures, scope, "device_type", lambda: device_type_name(core, label))
        names = _query(failures, scope, "property_names", lambda: sorted(_strings(core.get_device_property_names(label))))
        properties = [] if names is None else [_property_record(core, label, p, failures) for p in names]
        state_labels = []
        if kind == "StateDevice":
            state_labels = _query(failures, scope, "state_labels", lambda: _strings(core.get_state_labels(label)), [])
        devices.append({
            "label": label, "device_type": kind,
            "adapter": {
                "library": _query(failures, scope, "adapter_library", lambda: str(core.get_device_library(label))),
                "name": _query(failures, scope, "adapter_name", lambda: str(core.get_device_adapter_name(label))),
                "description": _query(failures, scope, "description", lambda: str(core.get_device_description(label))),
            },
            "properties": properties, "state_labels": sorted(state_labels),
        })

    groups = []
    for group in _query(failures, "core", "configuration_groups", lambda: sorted(_strings(core.get_available_config_groups())), []):
    ...
    facts = {
        "core_identity": {
            "version": _query(failures, "core", "version", lambda: str(core.get_version_info())),
            "api_version": _query(failures, "core", "api_version", lambda: str(core.get_api_version_info())),
        },
